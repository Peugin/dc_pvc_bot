...
#                 await message.channel.send(
#                     f"{message.author.mention} 嘗試搶奪 {target_user.display_name} 時，地雷卡爆炸了！你的全部 {user_points} 點數被轉移到 {target_user.display_name} 那裡！")
#                 return
#
#         if chance <= 40:
#             # 成功，取得對方10%點數
...
#                 if explosion_chance < 0.25:  # 假設有25%的爆炸機率
#                     # 將搶奪者的所有點數轉移給被搶奪者
#                     c.execute("UPDATE user_points SET points = points + ? WHERE user_id = ?",
...
#                     del landmine_status[target_user_id]
#                     await message.channel.send(
#                         f"{message.author.mention} 嘗試奴役 {target_user.display_name} 時，地雷卡爆炸了！你的全部 {user_points} 點數被轉移到 {target_user.display_name} 那裡！")
#                     return
#
#             # 奴役機率
...
#                 if target_user_id in angel_immunity:
#                     await message.channel.send(
#                         f"{message.author.mention} 目標用戶 {target_user.display_name} 目前有天使卡免疫，奴役失敗！")
#                     del angel_immunity[target_user_id]
#                     return
#
...
#                 c.execute("INSERT INTO user_cards (user_id, card_id, card_name, quantity) VALUES (?, ?, ?, 1)",
...
#             # 回覆抽卡結果
#             await message.channel.send(
#                 f"{message.author.mention} 消耗了 90% 的點數，抽到了一張 **{card_name}**！目前剩餘點數：{updated_points}")
#
#         else:
#             # 點數不足時的回覆
...
#             c.execute("UPDATE user_points SET points = ? WHERE user_id = ?", (new_points, user_id))
#             # 更新卡片數量
#             c.execute("UPDATE user_cards SET quantity = quantity - ? WHERE user_id = ? AND card_id = 1",
#                       (quantity, user_id))
#             conn.commit()
#
#             await message.channel.send(
#                 f"{message.author.mention} 使用了 {quantity} 張點數卡，獲得 {points_gained} 點，現在擁有 {new_points} 點數！")
#         else:
...
#
#         # 查詢用戶擁有的卡片數量
#         c.execute("SELECT quantity FROM user_cards WHERE user_id = ? AND card_id = ?",
#                   (user_id, 2))  # 假設天使卡的 card_id 是 2
#         result = c.fetchone()
#
#         if result and result[0] > 0:
#             # 使用天使卡，減少卡片數量
...
#                     # 啟用防B並設定5回合
#                     c.execute(
#                         "INSERT OR REPLACE INTO user_defense (user_id, has_defense, rounds_left) VALUES (?, 1, 5)",
#                         (user_id,))
#                     conn.commit()
...
#                     if rounds_left <= 0:
#                         # 回合數結束，移除防B
#                         c.execute("UPDATE user_defense SET has_defense = 0, rounds_left = 0 WHERE user_id = ?",
#                                   (user_id,))
...
#
#             # 發送抽卡結果
#             response_message = f"{message.author.mention} 抽卡內容:\t" + "\t".join(
#                 [f"<:{result.split(':')[1]}:{result.split(':')[2]}>" for result in results])
#
#             # 如果抽到，顯示額外訊息
...
#                 c.execute("UPDATE user_defense SET has_defense = ?, rounds_left = ? WHERE user_id = ?",
#                           (*updated_defense_status, user_id))
#             conn.commit()
#
#         # 更新積分和防B狀態
...
#             # 發送賭博結果
#             await message.channel.send(
#                 f"{message.author.mention} 你賭博的結果是: **{gamble_result}**。\n目前積分: {updated_points} 點。")
#         else:
#             await message.channel.send(f"{message.author.mention} 你目前沒有積分紀錄。")
#
...
#
#                 await message.channel.send(
#                     f"{message.author.mention} 機率 {success_rate_percent}% 強化成功！\n裝備 {equipment_name} 強化至 {new_upgrade} 級。屬性已增加 10%。")
#             else:
#                 # 強化失敗
#                 await message.channel.send(
#                     f"{message.author.mention} 機率 {success_rate_percent}% 強化失敗，裝備 {equipment_name} 依然保持原狀。")
#
#         except ValueError:
...
#         # 將卡片數量增加到用戶的卡片庫
#         c.execute(
#             "INSERT INTO user_cards (user_id, card_id, card_name, quantity) VALUES (?, ?, ?, ?) ON CONFLICT(user_id, card_id) DO UPDATE SET quantity = quantity + ?",
